...
Find the task's folder under `tasks/` (the most recently modified `tasks/<date>-<task-name>/` containing a `todo.md`; if more than one is a plausible match, ask the user which task this is for) and pick the next pending task from its `todo.md`. Then:
...
1. **Require a spec.** Look only for a spec at a known path: `tasks/<date>-<task-name>/SPEC.md`. A README or arbitrary doc does **not** count. If none exists, stop and tell the user to run `/spec` first — do not invent requirements. If more than one task folder is a plausible match, ask the user which one.
2. **Establish a clean baseline.** Run `git status --porcelain`. If there are uncommitted changes outside the expected planning artifacts (the task's `tasks/<date>-<task-name>/` folder), stop and ask the user to commit, stash, or confirm how to handle them. Autonomous per-task commits must not absorb unrelated local work, or the clean-rollback guarantee breaks.
3. **Plan if needed.** If the task folder has no `plan.md`, invoke agent-skills:planning-and-task-breakdown to generate one in that same folder.
4. **Single checkpoint.** Present the full plan and wait for an unambiguous affirmative (e.g. "approve", "go", "yes"). Treat hedged responses ("looks reasonable", "I guess") as **not** approved. This is the only human gate — after approval, run autonomously. If you generated `plan.md`, commit it as a single preparatory commit now so it doesn't bleed into the first task's commit.
...
6. **Stop and ask the user** (do not push through) when:
   - a test can't be made to pass or the build breaks without an obvious fix — diagnose the root cause before retrying; don't guess-and-check
   - the spec is ambiguous, or a task needs a decision the spec doesn't cover
   - a task is high-risk or irreversible — auth/permission changes, destructive data migrations, payments, deletions, deploys, anything touching secrets, **or anything you can't undo with `git revert`** — get explicit sign-off before continuing
